...
def Im2Patch(img, win, stride=1):
    ...
    endw = img.shape[1]
    endh = img.shape[2]
    patch = img[:, 0:endw - win + 0 + 1:stride,
            0:endh - win + 0 + 1:stride]  # 1 0:256-128+1:assssssssssssssssssssssssssssssssssssssssss
    # print(patch.shape[1], patch.shape[2])
...
# IR_files = sorted(get_img_file(r"Medical_train/ir"))
# VIS_files = sorted(get_img_file(r"Medical_train/vi"))

assert len(IR_files) == len(VIS_files)
h5f = h5py.File(os.path.join('./data',
                             data_name + '_imgsize2_' + str(img_size) + "_stride2_" + str(stride) + '.h5'),
                'w')
...
for i in tqdm(range(len(IR_files))):
    ...
    I_IR = imread(IR_files[i]).astype(np.float32).transpose(2, 0, 1) / 255
    I_IR = I_IR[0, :, :][np.newaxis, :, :]
    print(I_IR.shape)
    ###############################################################################
...
with h5py.File(os.path.join('data',
                            data_name + '_imgsize2_' + str(img_size) + "_stride2_" + str(stride) + '.h5'), "r") as f:
    for key in f.keys():
